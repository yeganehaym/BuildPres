...
if ( ! class_exists( 'TGM_Plugin_Activation' ) ) {
    ...
    class TGM_Plugin_Activation {
        ...
        public function __construct() {
            ...
            $this->strings = array(
                'page_title'                     => __( 'نصب افزونه‌های اجباری', 'buildpress_wp' ),
                'menu_title'                     => __( 'نصب افزونه‌ها', 'buildpress_wp' ),
                'installing'                     => __( 'درحال نصب افزونه: %s', 'buildpress_wp' ),
                'oops'                           => __( 'خطایی رخ داد.', 'buildpress_wp' ),
                'notice_can_install_required'    => _n_noop( 'نصب این افزونه برای این قالب اجباری است: %1$s.', 'نصب این افزونه ها برای این قالب اجباری است: %1$s.' ),
                'notice_can_install_recommended' => _n_noop( 'این قالب افزونه مقابل را پیشنهاد میکند: %1$s.', 'این قالب افزونه‌های مقابل را پیشنهاد میکند: %1$s.' ),
                'notice_cannot_install'          => _n_noop( 'Sorry, but you do not have the correct permissions to install the %s plugin. Contact the administrator of this site for help on getting the plugin installed.', 'Sorry, but you do not have the correct permissions to install the %s plugins. Contact the administrator of this site for help on getting the plugins installed.' ),
                'notice_can_activate_required'   => _n_noop( 'افزونه ضروری مقابل غیرفعال است: %1$s.', 'افزونه های ضروری مقابل غیرفعال هستند: %1$s.' ),
                'notice_can_activate_recommended'=> _n_noop( 'افزونه پیشنهادی مقابل غیرفعال است: %1$s.', 'افزونه‌های پیشنهادی مقابل غیرفعال است: %1$s.' ),
                ...
                'install_link'                   => _n_noop( 'شروع نصب افزونه', 'شروع نصب افزونه ها' ),
                'activate_link'                  => _n_noop( 'شروع فعال سازی افزونه', 'شروع فعال سازی افزونه‌ها' ),
                'return'                         => __( 'بازگشت به نصب افزونه های ضروری', 'buildpress_wp' ),
                'dashboard'                      => __( 'بازگشت به پیشخوان', 'buildpress_wp' ),
                'plugin_activated'               => __( 'افزونه با موفقیت فعال گردید.', 'buildpress_wp' ),
                'activated_successfully'         => __( 'افزونه مقابل با موفقیت فعال شد:', 'buildpress_wp' ),
                'complete'                       => __( 'تمامی افزونه های با موفقیت نصب و فعال شدند. %1$s', 'buildpress_wp' ),
                'dismiss'                        => __( 'نادیده گرفتن', 'buildpress_wp' ),
            );
        ...
        protected function do_plugin_install() {
            ...
            if ( isset( $_GET['plugin'] ) && ( isset( $_GET['tgmpa-install'] ) && 'install-plugin' == $_GET['tgmpa-install'] ) ) {
                ...
                if ( empty( $complete ) ) {
                    echo '<p>' .  sprintf( $this->strings['complete'], '<a href="' . network_admin_url() . '" title="' . __( 'Return to the Dashboard', 'buildpress_wp' ) . '">' . __( 'Return to the Dashboard', 'buildpress_wp' ) . '</a>' ) . '</p>';
        ...
        public function notices() {
            ...
            if ( ! empty( $message ) ) {
                ...
                foreach ( $message as $type => $plugin_groups ) {
                    ...
                    $rendered .= '<p>' . sprintf( translate_nooped_plural( $this->strings[$type], $count, 'buildpress_wp' ), $imploded, $count ) . '</p>';
                }

                // Setup variables to determine if action links are needed.
                $show_install_link  = $install_link ? '<a href="' . add_query_arg( 'page', $this->menu, network_admin_url( 'themes.php' ) ) . '">' . translate_nooped_plural( $this->strings['install_link'], $install_link_count, 'buildpress_wp' ) . '</a>' : '';
                $show_activate_link = $activate_link ? '<a href="' . add_query_arg( 'page', $this->menu, network_admin_url( 'themes.php' ) ) . '">' . translate_nooped_plural( $this->strings['activate_link'], $activate_link_count, 'buildpress_wp' ) . '</a>'  : '';
                ...
                if ( ! empty( $this->strings['nag_type'] ) ) {
                    add_settings_error( 'buildpress_wp', 'buildpress_wp', $rendered, sanitize_html_class( strtolower( $this->strings['nag_type'] ) ) );
                } else {
                    add_settings_error( 'buildpress_wp', 'buildpress_wp', $rendered, $nag_class );
                }
            }

            // Admin options pages already output settings_errors, so this is to avoid duplication.
            if ( 'options-general' !== $current_screen->parent_base ) {
                settings_errors( 'buildpress_wp' );
            }
...
if ( ! function_exists( 'buildpress_wp' ) ) {
    /**
    ...
    class TGMPA_List_Table extends WP_List_Table {
            ...
            foreach ( TGM_Plugin_Activation::$instance->plugins as $plugin ) {
                ...
                if ( ! empty( $plugin['source'] ) ) {
                    ...
                    if ( preg_match( '|^http(s)?://|', $plugin['source'] ) ) {
                        $table_data[$i]['source'] = __( 'Private Repository', 'buildpress_wp' );
                    // The plugin is pre-packaged with the theme.
                    } else {
                        $table_data[$i]['source'] = __( 'Pre-Packaged', 'buildpress_wp' );
                    }
                }
                // The plugin is from the WordPress repository.
                else {
                    $table_data[$i]['source'] = __( 'WordPress Repository', 'buildpress_wp' );
                }

                $table_data[$i]['type'] = isset( $plugin['required'] ) && $plugin['required'] ? __( 'Required', 'buildpress_wp' ) : __( 'Recommended', 'buildpress_wp' );

                if ( ! isset( $installed_plugins[$plugin['file_path']] ) ) {
                    $table_data[$i]['status'] = sprintf( '%1$s', __( 'Not Installed', 'buildpress_wp' ) );
                } elseif ( is_plugin_inactive( $plugin['file_path'] ) ) {
                    $table_data[$i]['status'] = sprintf( '%1$s', __( 'Installed But Not Activated', 'buildpress_wp' ) );
                }
        ...
        public function column_plugin( $item ) {
            ...
            if ( ! isset( $installed_plugins[$item['file_path']] ) ) {
                $actions = array(
                    'install' => sprintf(
                        '<a href="%1$s" title="' . __( 'Install', 'buildpress_wp' ) . ' %2$s">' . __( 'Install', 'buildpress_wp' ) . '</a>',
                        wp_nonce_url(
                            add_query_arg(
                                array(
            ...
            elseif ( is_plugin_inactive( $item['file_path'] ) ) {
                $actions = array(
                    'activate' => sprintf(
                        '<a href="%1$s" title="' . __( 'Activate', 'buildpress_wp' ) . ' %2$s">' . __( 'Activate', 'buildpress_wp' ) . '</a>',
                        add_query_arg(
                            array(
                                'page'                 => TGM_Plugin_Activation::$instance->menu,
                                'plugin'               => $item['slug'],
        ...
        public function no_items() {
            ...
            printf( __( 'No plugins to install or activate. <a href="%1$s" title="Return to the Dashboard">Return to the Dashboard</a>', 'buildpress_wp' ), network_admin_url() );
            echo '<style type="text/css">#adminmenu .wp-submenu li.current { display: none !important; }</style>';
        ...
        public function get_columns() {
            ...
            $columns = array(
                ...
                'plugin' => __( 'Plugin', 'buildpress_wp' ),
                'source' => __( 'Source', 'buildpress_wp' ),
                'type'   => __( 'Type', 'buildpress_wp' ),
                'status' => __( 'Status', 'buildpress_wp' )
            );
        ...
        public function get_bulk_actions() {
            ...
            $actions = array(
                'tgmpa-bulk-install'  => __( 'Install', 'buildpress_wp' ),
                'tgmpa-bulk-activate' => __( 'Activate', 'buildpress_wp' ),
            );
        ...
        public function process_bulk_actions() {
            ...
            if ( 'tgmpa-bulk-activate' === $this->current_action() ) {
                ...
                if ( is_wp_error( $activate ) ) {
                    ...
                    printf( '<div id="message" class="updated"><p>%1$s %2$s</p></div>', _n( 'The following plugin was activated successfully:', 'The following plugins were activated successfully:', $count, 'buildpress_wp' ), $imploded );
                }
    ...
    if ( ! class_exists( 'TGM_Bulk_Installer' ) ) {
        ...
        class TGM_Bulk_Installer extends WP_Upgrader {
            ...
            public function install_strings() {
                ...
                $this->strings['no_package']          = __( 'Install package not available.', 'buildpress_wp' );
                $this->strings['downloading_package'] = __( 'Downloading install package from <span class="code">%s</span>&#8230;', 'buildpress_wp' );
                $this->strings['unpack_package']      = __( 'Unpacking the package&#8230;', 'buildpress_wp' );
                $this->strings['installing_package']  = __( 'Installing the plugin&#8230;', 'buildpress_wp' );
                $this->strings['process_failed']      = __( 'Plugin install failed.', 'buildpress_wp' );
                $this->strings['process_success']     = __( 'Plugin installed successfully.', 'buildpress_wp' );
            ...
            public function activate_strings() {
                ...
                $this->strings['activation_failed']  = __( 'Plugin activation failed.', 'buildpress_wp' );
                $this->strings['activation_success'] = __( 'Plugin activated successfully.', 'buildpress_wp' );
    ...
    if ( ! class_exists( 'TGM_Bulk_Installer_Skin' ) ) {
        ...
        class TGM_Bulk_Installer_Skin extends Bulk_Upgrader_Skin {
            ...
            public function add_strings() {
                ...
                if ( TGM_Plugin_Activation::$instance->is_automatic ) {
                    $this->upgrader->strings['skin_upgrade_start']        = __( 'The installation and activation process is starting. This process may take a while on some hosts, so please be patient.', 'buildpress_wp' );
                    $this->upgrader->strings['skin_update_successful']    = __( '%1$s installed and activated successfully.', 'buildpress_wp' ) . ' <a onclick="%2$s" href="#" class="hide-if-no-js"><span>' . __( 'Show Details', 'buildpress_wp' ) . '</span><span class="hidden">' . __( 'Hide Details', 'buildpress_wp' ) . '</span>.</a>';
                    $this->upgrader->strings['skin_upgrade_end']          = __( 'All installations and activations have been completed.', 'buildpress_wp' );
                    $this->upgrader->strings['skin_before_update_header'] = __( 'Installing and Activating Plugin %1$s (%2$d/%3$d)', 'buildpress_wp' );
                }
                // Default installation strings.
                else {
                    $this->upgrader->strings['skin_upgrade_start']        = __( 'The installation process is starting. This process may take a while on some hosts, so please be patient.', 'buildpress_wp' );
                    $this->upgrader->strings['skin_update_failed_error']  = __( 'An error occurred while installing %1$s: <strong>%2$s</strong>.', 'buildpress_wp' );
                    $this->upgrader->strings['skin_update_failed']        = __( 'The installation of %1$s failed.', 'buildpress_wp' );
                    $this->upgrader->strings['skin_update_successful']    = __( '%1$s installed successfully.', 'buildpress_wp' ) . ' <a onclick="%2$s" href="#" class="hide-if-no-js"><span>' . __( 'Show Details', 'buildpress_wp' ) . '</span><span class="hidden">' . __( 'Hide Details', 'buildpress_wp' ) . '</span>.</a>';
                    $this->upgrader->strings['skin_upgrade_end']          = __( 'All installations have been completed.', 'buildpress_wp' );
                    $this->upgrader->strings['skin_before_update_header'] = __( 'Installing Plugin %1$s (%2$d/%3$d)', 'buildpress_wp' );
                }
            ...
            public function bulk_footer() {
                ...
                if ( empty( $complete ) ) {
                    echo '<p>' .  sprintf( TGM_Plugin_Activation::$instance->strings['complete'], '<a href="' . network_admin_url() . '" title="' . __( 'Return to the Dashboard', 'buildpress_wp' ) . '">' . __( 'Return to the Dashboard', 'buildpress_wp' ) . '</a>' ) . '</p>';
                    echo '<style type="text/css">#adminmenu .wp-submenu li.current { display: none !important; }</style>';
                }
